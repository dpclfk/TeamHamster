    ...
    if (
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          navigate("/report"); // 로그인 성공시 홈으로 이동합니다.
        } else {
          setLoginCheck(true);
        }
      } catch (error) {
    ...
    <div>
      <div className={` ${box} ${center}`}>
        <div className="rounded-lg  w-full m">
          ...
          <h2 className="text-2xl font-bold text-center mb-10">관리자 로그인</h2>
          <form onSubmit={handleLogin}>
            ...
            <div className="mb-10" onClick={handleLogin}>
              <LargeButton btn={new Button("로그인", "bg-orange-500 w-auto")}></LargeButton>
            </div>
          </form>
        </div>
      </div>
